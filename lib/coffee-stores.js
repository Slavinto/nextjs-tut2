...
const getUnsplashImgUrls = async (limit) => {
...
export const fetchCoffeeStores = async (
    ...
    // console.log({ coords, fromClient, limit });
    const options = {
        method: "GET",
        headers: {
    ...
    try {
        ...
        let imgUrls = await getUnsplashImgUrls(limit);

        if (imgUrls.length === 0)
            imgUrls = Array.from({ length: +limit * 2 }, () => defaultImgUrl);
